...
const oobx = x => ((x < 0) ? -x : (x < Vars.world.width() ? 0 : (x
    - Vars.world.width())))
const ooby = y => ((y < 0) ? -y : (y < Vars.world.height() ? 0 : (y
    - Vars.world.height())))

const timer = new Interval()

Events.run(Trigger.update, () => {
  if (Vars.state.rules.pvp && timer.get(Time.toSeconds / 2)) {
    Groups.player.each(p => {
      let distance = Math.hypot(oobx(p.tileX()), ooby(p.tileY()));
      if (distance > 0) {
        Call.announce(p.con(),
            "[red]You are out of the map bounds.\nYou will loose health!")
