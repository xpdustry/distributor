...
final class MenuManagerImpl extends AbstractTransformerWindowManager<MenuPane> implements MenuManager {
    ...
    protected void onWindowOpen(final SimpleWindow window) {
        final var options = window.getPane().grid().getOptions().stream()
                .map(row -> row.stream()
        ...
        Call.followUpMenu(
                ...
                this.id,
                this.render(window, window.getPane().title()),
                this.render(window, window.getPane().content()),
                options);
    }
    ...
    private void handle(final Player player, final int option) {
        ...
        if (window == null) {
            ...
            window.getPane().exit().act(window);
        } else {
            final var choice = window.getPane().grid().getOption(option);
            if (choice == null) {
                this.getPlugin()
                        .getLogger()
                        .debug(
