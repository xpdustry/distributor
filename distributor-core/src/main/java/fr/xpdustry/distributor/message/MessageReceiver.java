...
public interface MessageReceiver {
  ...
  static MessageReceiver ofPlayers(final @NotNull Iterable<Player> players) {
    return new ForwardingMessageReceiver(new ArcList<>(Seq.with(players).map(ClientMessageReceiver::new)));
  }

  static MessageReceiver ofReceivers(final @NotNull Iterable<MessageReceiver> receivers) {
    return new ForwardingMessageReceiver(new ArcList<>(Seq.with(receivers)));
  }

  static MessageReceiver server() {
    return ServerMessageReceiver.getInstance();
  }
